...
(function ($) {
    ...
    var carousel = (function () {
        ...
        var carousel = function (containerEl, opts) {
            ...
            try {
                ...
                if (this.container.style.width) { this.myDivWidth = numOnly(this.container.style.width); }
                else {
                    alert("Please set the width on " + this.container.id + " object to use the carousel script");
                    return;
                }
                if (this.container.style.height) { this.myDivHeight = numOnly(this.container.style.height); }
                else {
                    alert("Please set the height on " + this.container.id + " object to use the carousel script");
                    return;
                }
                // initial setup
                ...
                for (; n; n = n.nextSibling) {
                    if (n.nodeType === 1) { childrenCounter++; }
                }
                ...
                el.style.width = Math.ceil((this.childrenCount) * this.myDivWidth) + "px";
                ...
                if (this.pagingDiv) {
                    for (i = 0; i < this.childrenCount; i++) {
                        ...
                        pagingEl.pageId = i;
                        if (i !== 0) { pagingEl.className = this.pagingCssName; }
                        else { pagingEl.className = this.pagingCssNameSelected; }
        ...
        carousel.prototype = {
            ...
            touchEnd: function (e) {
                ...
                try {
                    ...
                    if (endPos > 0) {
                        ...
                        if (endPos < this.cssMoveStart && totalMoved > 10) {
                            currInd++; // move right/down
                        } else if ((endPos > this.cssMoveStart && totalMoved < 90)) {
                            currInd--; // move left/up
                        }
                        if (currInd > (this.childrenCount - 1)) { currInd = this.childrenCount - 1; }
                        if (currInd < 0) { currInd = 0; }
                        var movePos = {
                            x: 0,
                            y: 0
                        };
                        if (this.vertical) { movePos.y = (currInd * this.myDivHeight * -1); }
                        else { movePos.x = (currInd * this.myDivWidth * -1); }

                        this.moveCSS3(this.el, movePos, "150");

                        if (this.pagingDiv && this.carouselIndex !== currInd) {
                            document.getElementById(this.container.id + "_" + this.thisCarouselIndex).className = this.pagingCssName;
            ...
            touchMove: function (e) {
                ...
                //moved movePos to inside the carousel type conditional, due to the carousel reseting on touchMove
            ...
            onMoveIndex: function OnMoveIndex(newInd) {
                try {
                    document.getElementById(this.container.id + "_" + this.thisCarouselIndex).className = this.pagingCssName;
                    var newTime = Math.abs(newInd - this.thisCarouselIndex);

                    var ind = newInd;
                    if (ind < 0) ind = 0;
                    if (ind > this.childrenCount - 1) { ind = this.childrenCount - 1; }
                    var movePos = {
                        x: 0,
                        y: 0
                    };
                    if (this.vertical) { movePos.y = (ind * this.myDivHeight * -1); }
                    else { movePos.x = (ind * this.myDivWidth * -1); }

                    var time = 50 + parseInt((newTime * 20));
                    this.moveCSS3(this.el, movePos, time);
                    this.thisCarouselIndex = ind;
                    if (this.pagingDiv) {
                        document.getElementById(this.container.id + "_" + this.thisCarouselIndex).className = this.pagingCssNameSelected;
                    }
                } catch (e) {
                    console.log("Error " + e);
                }
    ...
    function isHorizontalSwipe(xAxis, yAxis) {
        ...
        if (swipeAngle < 0) { swipeAngle = 360 - Math.abs(swipeAngle); } // correct for negative degree values
